...
def prosail_shell_v2(
    ...
    with torch.no_grad():
        if traits.ndim == 1:
            ...
            N = N.unsqueeze(0)
            # Use the 'tran_alpha' as the alpha parameter for PROSPECT.
            rho, tau = prospect(traits, N, alpha=tran_alpha, device=device)
            out = foursail_main(
                rho, tau, psoil, LIDFa, LIDFb, lai, q,
                tts, tto, psi, rd, pi_val, litab,
                dry_soil, wet_soil,
                use_batch=False, lidtype=lidtype
            )
            refl_mat = torch.stack(out, dim=1).detach().cpu().numpy()
            return pd.DataFrame(refl_mat, columns=['rddt', 'rsdt', 'rdot', 'rsot', 'tsd', 'tdd', 'rdd'])
        else:
            ...
            results = []
            for batch in loader:
                b_traits, b_N, b_LIDFa, b_LIDFb, b_lai, b_q, b_tts, b_tto, b_psi, b_psoil, b_tran_alpha = batch
                rho, tau = prospect(b_traits, b_N, alpha=b_tran_alpha, device=device)
                out = foursail_main(
                    rho, tau, b_psoil, b_LIDFa, b_LIDFb, b_lai, b_q,
                    b_tts, b_tto, b_psi,
                    rd, pi_val, litab, dry_soil, wet_soil,
                    use_batch=True, lidtype=lidtype
                )
