...
def foursail_main(
    ...
    dry_soil=None, wet_soil=None,
    use_batch=True, lidtype=1
):
    """
    ...
    len_na = len(na)
    len_batch = lai.shape[0] if use_batch else 0
    ...
    ks, ko, sob, sof, sdb, sdf, dob, dof, ddb, ddf = SUITS(
        na, litab, lidFun, tts, tto, cts, cto, psi, ctscto,
        use_batch=use_batch, len_batch=len_batch, len_na=len_na
    )
    ...
    rdd, tdd, tsd, rsd, tdo, rdo, tss, too, rsod = ReflTrans(
        rho, tau, lai, att, m, sigb, ks, ko, sf, sb, vf, vb, use_batch=use_batch
    )

    # Hotspot effect
    from torchrtm.canopy.hotspot import HotSpot
    tsstoo, sumint = HotSpot(lai, q, tss, ks, ko, dso, use_batch=use_batch)

    # Final BRDF computation
    finalOut = sail_BDRF(
        w, lai, sumint, tsstoo, rsoil,
        rdd, tdd, tsd, rsd, tdo, rdo, tss, too, rsod,
        len_batch=len_batch, use_batch=use_batch
    )
